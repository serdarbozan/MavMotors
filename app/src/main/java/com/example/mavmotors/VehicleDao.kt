...
interface VehicleDao
{
    ...
    @Query("SELECT * FROM vehicles WHERE status != 'Sold' ORDER BY postedDate DESC")
    suspend fun getAllVehicles(): List<Vehicle>

    @Query("SELECT * FROM vehicles WHERE id = :id")
    suspend fun getVehicleById(id: Int): Vehicle?

    @Query("SELECT * FROM vehicles WHERE type = :type AND status != 'Sold' ORDER BY postedDate DESC")
    ...
    @Query("SELECT * FROM vehicles WHERE sellerId != :userId AND status != 'Sold' ORDER BY postedDate DESC")
    ...
    @Query("SELECT * FROM vehicles WHERE sellerId != :userId AND type = :type AND status != 'Sold' ORDER BY postedDate DESC")
    ...
    @Query("UPDATE vehicles SET status = 'Sold' WHERE id = :vehicleId")
    suspend fun markVehicleAsSold(vehicleId: Int)
}